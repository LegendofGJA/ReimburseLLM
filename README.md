...
- `gps_core.py` — baca lokasi GPS dari EXIF foto struk + reverse geocode (Nominatim)
...
- `.devcontainer/devcontainer.json` — environment VS Code / Codespaces (Python 3.11 + auto-run Streamlit)
- `.github/workflows/keep-alive.yml` — ping opsional ke URL app (set repository variable `APP_URL` untuk mengaktifkan)

## Catatan privasi

- Template Excel dan default sidebar **tidak** menyimpan data pribadi —
  Name/Department/Bank Acc. diisi lewat sidebar tiap kali buka app, atau
  lewat Streamlit Secrets (opsional: `DEFAULT_NAME`, `DEFAULT_DEPARTMENT`,
  `DEFAULT_PURPOSE`, `DEFAULT_BANK_ACC`).
- Foto struk dan screenshot yang diunggah **tidak pernah disimpan** ke
  disk/repo; hanya diproses di memori dan dikirim ke provider AI vision yang
  kamu konfigurasi sendiri.
- Satu-satunya layanan eksternal non-AI adalah Nominatim (OpenStreetMap) untuk
  reverse geocode koordinat GPS EXIF → nama lokasi parkir.
